...
function M.get_response(c_id, callback)
  ...
  local payload = { model = config.get_user_config().model, messages = convo.messages }
  ...
  Job:new({
    ...
    on_exit = vim.schedule_wrap(function(job, exit_code)
      if exit_code ~= 0 then
        vim.notify(string.format("[fast_agent.nvim] HTTP error (curl exit code %d)", exit_code), vim.log.levels.ERROR)
        return
      end

      local result = table.concat(job:result(), "")
      if result == "" then
        vim.notify("[fast_agent.nvim] Empty response from OpenAI API.", vim.log.levels.ERROR)
        return
      end

      local ok, decoded = pcall(vim.fn.json_decode, result)
      if not ok or type(decoded) ~= "table" or not decoded.choices then
        vim.notify("[fast_agent.nvim] Failed to parse OpenAI JSON response.", vim.log.levels.ERROR)
        return
      end

      local choice = decoded.choices[1]
      if not choice or not choice.message or not choice.message.content then
        vim.notify("[fast_agent.nvim] Unexpected OpenAI response structure.", vim.log.levels.ERROR)
        return
      end

      local assistant_text = choice.message.content
      table.insert(convo.messages, { role = "assistant", content = assistant_text })
